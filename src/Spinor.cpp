...
Spinor::Spinor(int n, complex<double> a, complex<double> b, complex<double> c) {
    if (n != 3) {
        cout
            << "error: this constructor [Spinor::Spinor(int n, complex<double> "
               "a, "
               "complex<double> b, complex<double> c)] only works for SU(3)... "
               "exiting."
            << endl;
        exit(1);
    }
...
Spinor::Spinor(int n, complex<double> a, complex<double> b) {
    if (n != 2) {
        cout << "error: this constructor [Spinor::Spinor(int n, "
                "complex<double> a, "
                "complex<double> b)] only works for SU(2)... exiting."
             << endl;
        exit(1);
    }
    ndim = n;
...
Spinor operator%(const Spinor &a, const Spinor &b) {
    if (a.getNDim() == 3)
        return Spinor(
            3, conj(a(1) * b(2) - a(2) * b(1)), conj(a(2) * b(0) - a(0) * b(2)),
            conj(a(0) * b(1) - a(1) * b(0)));
    else {
